...
def get_angles(arm_north):
    ...
    arm_theta = np.arccos(np.dot(arm_north, [0, 0, 1])) * 180 / np.pi
    arm_phi = np.arccos(np.dot(arm_north, [0, 0, -1])) * 180 / np.pi

    return arm_theta, arm_phi
