...
def microbit_data_to_dataframe(microbit_data: dict[Characteristic, np.ndarray]):
    ...
    for characteristic, data in microbit_data.items():
        if (
            characteristic == Characteristic.ACCELEROMETER
            or characteristic == Characteristic.MAGNETOMETER
        ):
            print(data)
...
if __name__ == "__main__":
    ...
    for n, data in enumerate(result):
        microbit_data_to_dataframe(data).to_excel("microbit_data_0.xlsx", index=False)
