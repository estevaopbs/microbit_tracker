...
class MicrobitRecorder:
    def __init__(self, microbit: KaspersMicrobit, characteristics):
        ...
        if Characteristic.IOPIN in characteristics:
            num_inputs = str(self.microbit.io_pin.read_io_configuration()).count(
                "PinIO.INPUT"
            )
            if num_inputs > 0:
    ...
    def actually_update_button_a(self):
        self.data[Characteristic.BUTTONA].append(self.microbit.buttons.read_button_a())

    def actually_update_button_b(self):
        self.data[Characteristic.BUTTONB].append(self.microbit.buttons.read_button_b())

    def actually_update_temperature(self):
        self.data[Characteristic.TEMPERATURE].append(self.microbit.temperature.read())

    def actually_update_io_pin(self):
        self.data[Characteristic.IOPIN].append(self.microbit.io_pin.read_data())
...
def record_microbits(
    *microbits,
    ...
    """
    characteristics = set([characteristic for characteristic in characteristics])
    recorders = [MicrobitRecorder(microbit, characteristics) for microbit in microbits]
    if verbose:
        verbose_fn = print
    else:
    ...
    for recorder in recorders:
        ...
        for characteristic in recorder.data.keys():
            recorder.data[characteristic] = np.array(recorder.data[characteristic])
            if characteristic == "TIME":
                recorder.data[characteristic] = (
                    recorder.data[characteristic]
                    - start_time
                    - (time_list[0] - start_time)
                )
...
def format_data(recorder):
    return (
        f"MAGNETOMETER: {np.linalg.norm(recorder.data[Characteristic.MAGNETOMETER][-1])}\n"
        ...
        + f"ACCELEROMETER: {np.linalg.norm(recorder.data[Characteristic.ACCELEROMETER][-1])}\n"
        + f"ANGLE_X_ACC: {angle(recorder.data[Characteristic.ACCELEROMETER][-1], [1, 0, 0])}\n"
        + f"ANGLE_Y_ACC: {angle(recorder.data[Characteristic.ACCELEROMETER][-1], [0, 1, 0])}\n"
        + f"ANGLE_Z_ACC: {angle(recorder.data[Characteristic.ACCELEROMETER][-1], [0, 0, 1])}\n"
    )
